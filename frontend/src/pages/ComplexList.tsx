...
import { Plus, Trash2, ExternalLink, RefreshCw, Beaker, Pencil } from 'lucide-react'
...
export default function ComplexList() {
  ...
  const resetForm = () => {
    setShowForm(false)
    setEditingId(null)
    setFormData({ name: '', address: '', naverComplexId: '', tags: '', subwayLines: [] })
  }
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<Complex> }) => complexApi.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['complexes'] })
      resetForm()
    },
  ...
  const handleEdit = (complex: Complex) => {
    ...
    const subwayLines = tags.filter(tag => SUBWAY_LINES.some(line => tag.includes(line)))
    const otherTags = tags.filter(tag => !SUBWAY_LINES.some(line => tag.includes(line)))
  ...
  const handleCheckboxChange = (line: string, checked: boolean) => {
    if (checked) {
      setFormData(prev => ({ ...prev, subwayLines: [...prev.subwayLines, line] }))
    } else {
      setFormData(prev => ({ ...prev, subwayLines: prev.subwayLines.filter(l => l !== line) }))
    }
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const otherTags = formData.tags ? formData.tags.split(',').map(t => t.trim()).filter(t => t) : []
    // 중복 제거를 위해 Set 사용
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <Button 
            onClick={() => {
              if (confirm('365일간의 더미 데이터가 포함된 테스트 단지를 생성하시겠습니까?')) {
            ...
            variant="outline"
          >
            <Beaker className={`w-4 h-4 mr-2 ${createTestComplexMutation.isPending ? 'animate-spin' : ''}`} />
            {createTestComplexMutation.isPending ? '생성 중...' : '테스트 단지 생성'}
          </Button>
          <Button 
            onClick={() => scrapeAllMutation.mutate()}
            disabled={scrapeAllMutation.isPending || !complexes || complexes.length === 0}
            variant="default"
          >
      ...
      {showForm && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>네이버 부동산에서 단지 정보를 가져올 수 있습니다</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2">네이버 단지 ID (선택)</label>
                <Input
        ...
        {complexes?.map((complex) => {
          ...
          <Card key={complex.id} className="hover:shadow-lg transition-shadow">
            <CardHeader>
              ...
              <CardDescription className="text-xs">{complex.address}</CardDescription>
              {tags.length > 0 && (
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 py-2 border-y border-slate-100 text-xs">
                  <div className="flex flex-col">
                    ...
                    <span className="font-semibold text-sm">{complex.units || '-'}</span>
                  </div>
                  <div className="flex flex-col">
                ...
                <div className="py-2 text-xs">
                  ...
                  {complex.todayListingCounts && (
                    <div className="flex gap-2 justify-end text-[11px] text-slate-500">
                      <span className="text-red-500">매매 {complex.todayListingCounts.sale}</span>
                      <span className="text-blue-500">전세 {complex.todayListingCounts.jeonse}</span>
                      <span className="text-green-500">월세 {complex.todayListingCounts.rent}</span>
                    </div>
                  )}
                </div>
