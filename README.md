...
│                     Express 서버 (backend)                           │
│  ┌──────────────┐  ┌──────────────┐  ┌─────────────┐  ┌──────────┐ │
│  │ complexRoutes│  │listingRoutes │  │ statsRoutes  │  │backupRts │ │
│  │  단지 CRUD   │  │  매물 조회   │  │ 통계/트렌드  │  │ DB백업   │ │
│  │  스크래핑    │  │  삭제/필터   │  │ 신고가/저가  │  │ DB복구   │ │
│  └──────┬───────┘  └──────────────┘  └─────────────┘  └──────────┘ │
│         │                                                            │
│  ┌──────▼───────┐  ┌──────────────┐                                  │
│  │naverScraper  │  │   Prisma     │                                  │
│  │ Puppeteer    │◀─│  ORM 클라이언트│──▶ SQLite (dev.db)              │
│  │ 봇감지 우회   │  └──────────────┘                                  │
│  └──────────────┘                                                    │
│         │                                                            │
│  ┌──────▼───────┐                                                    │
│  │  parsers     │  네이버 부동산 API 응답 → ListingData 변환          │
│  └──────────────┘                                                    │
│                                                                      │
│  [정적 파일 서빙] ──▶ frontend/dist/ (React 빌드 결과물)              │
└──────────────────────────────────────────────────────────────────────┘
```
...
1. 매물 수집 흐름:
   [사용자: "매물 갱신" 클릭]
     → POST /api/complexes/:id/scrape
     → scrapeNaverListings() (Puppeteer로 네이버 부동산 접속)
     → API 응답 가로채기 (response 이벤트 리스너)
     → parseNaverArticle() → ListingData 배열 변환
     → 당일 기존 데이터 삭제 → createMany()로 DB 일괄 저장
     → lastScrapedAt 갱신

2. 데이터 조회 흐름:
   [대시보드 로드]
     → GET /api/complexes (최근일 매물 수, 증감, 신고가/저가 포함)
     → GET /api/stats/trend (전체 트렌드 + 요약 통계)
     → 프론트엔드에서 React Query로 캐싱 및 자동 갱신

3. 단지 상세 페이지:
   [ComplexDetail 접속]
     → GET /api/complexes/:id (단지 상세정보 + 통계)
     → GET /api/listings/:complexId (전체 매물 → 클라이언트 필터링)
     → ListingChart (Recharts로 시계열 차트 렌더링)
     → ListingTable (매물 목록 + 날짜/유형/면적 필터)
```
...
│ name          │ 단지명                             │
│ address       │ 주소                               │
│ naverComplexId│ 네이버 부동산 고유 ID (크롤링 키)    │
│ type          │ 유형 (아파트, 분양권 등)              │
│ units         │ 총 세대수                           │
│ buildings     │ 총 동수                             │
│ year          │ 준공년도                             │
│ approvalDate  │ 사용승인일                           │
│ areaOptions   │ 면적 옵션 리스트 (JSON)              │
│ tags          │ 지하철 호선/태그 (JSON)              │
│ customNotes   │ 사용자 메모                          │
│ lastScrapedAt │ 최종 매물 수집일                     │
│ infoScrapedAt │ 최종 정보 수집일                     │
│ createdAt     │ 생성일                               │
│ updatedAt     │ 수정일                               │
├──────────────────────────────────────────────────┤
│    1 : N                                         │
...
│──────────────────────────────────────────────────│
│ id            │ Int (PK, 자동증가)                 │
│ complexId     │ FK → Complex.id (CASCADE 삭제)     │
│ tradetype     │ 거래유형 (매매/전세/월세)             │
│ price         │ 가격 (만원 단위)                     │
│ area          │ 전용면적 (m²)                       │
│ supplyArea    │ 공급면적 (m²)                       │
│ floor         │ 층 정보                             │
